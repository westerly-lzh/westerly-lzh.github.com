(function() {
	...
	var testStyle = function(style) {
		...
		document.documentElement.className += ' ' + (bool ? '' : 'no-')
				+ style.replace(/-/g, '');
		return bool;
	};
	...
	Slide.prototype = {
		...
		_runAutos : function() {
			...
			if (idx >= 0) {
				...
				if (canTransition) {
					var l = function(evt) {
						elem.parentNode.removeEventListener(
								'webkitTransitionEnd', l, false);
						elem.parentNode.removeEventListener('transitionend', l,
								false); // moz
						elem.parentNode.removeEventListener('oTransitionEnd',
								l, false);
						_t._runAutos();
					};
					elem.parentNode.addEventListener('webkitTransitionEnd', l,
							false);
					elem.parentNode.addEventListener('transitionend', l, false);
					elem.parentNode
							.addEventListener('oTransitionEnd', l, false);
	...
	var SlideShow = function(slides) {
		...
		this.current = (!this.current) ? "landing-slide" : this.current
				.replace('#', '');
		...
		query('#left-init-key').addEventListener('click', function() {
			_t.next();
		}, false);
		this._update();
	...
	SlideShow.prototype = {
		...
		_update : function(targetId, dontPush) {
			...
			var elem = document.elementFromPoint(docElem.clientWidth / 2,
					docElem.clientHeight / 2);
			...
			this._speakerNote.innerHTML = this._slides[currentIndex - 1]
					.getSpeakerNote();
		...
		prev : function() {
			var prev = query('.slide:nth-child('
		...
		onCommandClick : function(e) {
			...
			switch (n) {
			case 'toc':
				this._update("table-of-contents");
				break;
			case 'resources':
				break;
			case 'notes':
